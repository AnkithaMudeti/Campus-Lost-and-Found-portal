...
import { FaBox, FaTimes, FaComments } from "react-icons/fa";
...
const FoundItemReport = () => {
  ...
  const handleChat = (reportedBy) => {
    // Navigate to chat page with the reported by user
    navigate(`/ChatPage?peer=${encodeURIComponent(reportedBy)}`);
  };
          ...
          <table className="min-w-full border border-gray-200 rounded-lg overflow-hidden">
            <thead className="bg-indigo-600 text-white">
              <tr>
                ...
                {/* Chat column only for Student */}
                {currentUser?.role === "Student" && (
                  <th className="px-4 py-3 text-left text-sm font-semibold">Chat</th>
                )}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 text-sm text-gray-700">
              {itemList.length > 0 ? (
                itemList.map((item) => (
                  <tr key={item.foundItemId} className="hover:bg-gray-50 transition">
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.foundItemId}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.itemName}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.category}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.color}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.brand}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.location}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.foundDate}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.username}</td>
                    <td className="px-4 py-3 cursor-pointer" onClick={() => setSelectedItem(item)}>{item.userEmail}</td>
                    {/* Chat button only for Student */}
                    {currentUser?.role === "Student" && (
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="bg-blue-600 text-white py-1.5 px-3 rounded-md hover:bg-blue-700 transition font-semibold flex items-center gap-1"
                ...
                <tr>
                  <td colSpan={currentUser?.role === "Student" ? "10" : "9"} className="px-4 py-6 text-center text-gray-500">No found items available.</td>
                </tr>
              )}
            </tbody>
          </table>
